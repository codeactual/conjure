module.exports = function() {
  ...
  var statusList = new StatusList(); // Index internal status JSON strings by type.
  ...
  */
  function runCasper(file, cb) {
    ...
    var stdout = self.createConsole(file, console.log);
    ...
    casper.stdout.on('data', function(data) {
      ...
      if (!statusList.push(data)) { // Store/parse status lines. Output remaining.
    ...
    if (self.options.timeout) {
      setTimeout(function() {
        ...
        var waitStatus = statusList.findLast(withType('wait'));
        if (waitStatus) {
  ...
  /**
   * findLast() truthy test.
   */
  function withType(type) {
    return function(status) { return status.type === type; };
  }
};
