...
function Process() {
  ...
  this.allStatus = statusList.create();
  this.rotatedStatus = statusList.create().set('rotationSource', 'it');
  this.timeoutId = null;
...
Process.prototype.spawn = function() {
  ...
  this.allStatus.set('impulseBin', this.get('impulseBin'));
  this.rotatedStatus.set('impulseBin', this.get('impulseBin'));
...
Process.prototype.onStdout = function(data) {
  ...
  if (this.allStatus.pushIfStatusFound(data)) { // Store/parse status lines. Output remaining.
    this.rotatedStatus.pushIfStatusFound(data);
  } else {
...
Process.prototype.showTrace = function() {
  var self = this;
  var fullTrace = this.get('cliOptions').fullTrace;
  var list = fullTrace ? this.allStatus : this.rotatedStatus;

  var traceSteps = list.select({type: 'trace'}).array();
  if (traceSteps.length) {
    var adjustDepth = fullTrace ? 0 : traceSteps[0].depth;
    this.stderr('conjure trace:');
    traceSteps.forEach(function conjureOnEachTraceStep(step) {
      self.stderr('    ' + step.toString(adjustDepth));
    });
  }
};
